...
# Load in data
chan_volt = pd.read_parquet('C:\\Users\\guido\\Data\\EphysAtlas\\channels_voltage_features.pqt')
...
for i, pid in enumerate(all_pids):
    ...
    traj = one.alyx.rest('trajectories', 'list', session=sess_info[0]['session'],
                         probe=sess_info[0]['name'], provenance='Micro-manipulator')
    if len(traj) != 1:
        continue
    mm_coord = pd.concat((mm_coord, pd.DataFrame(index=[pid], data={
        'x': traj[0]['x'],
        'y': traj[0]['y'],
        'depth': traj[0]['depth'],
        'theta': traj[0]['theta'],
