...
atlas_agea.plot_cslice(
    0, ax=axs[2, 0], volume=gene_expression_volumes[igenes[0]], cmap="viridis"
)
atlas_agea.plot_sslice(0, ax=axs[0, 1])
atlas_agea.plot_sslice(0, ax=axs[1, 1], volume="annotation")
atlas_agea.plot_sslice(
    0, ax=axs[2, 1], volume=gene_expression_volumes[igenes[0]], cmap="viridis"
)
fig.tight_layout()
...
sel = np.isin(
    atlas_agea.label, np.where(idx_beryl_root)[0]
)  # to select only root, lateralized
fig, axs = plt.subplots(3, 2, sharex=True, sharey=True)
...
sel = np.isin(
    atlas_agea.label.flatten(), np.where(idx_beryl_root)[0]
)  # to select only root, lateralized
...
custom_cmap = LinearSegmentedColormap.from_list(
    "custom_cmap", colors_ids_rgb / 255.0, N=len(colors_ids_rgb)
)

# Plot the scatter plot with the custom colormap
plt.figure(figsize=(12, 8))  # Adjust the values (width, height) as needed
plt.scatter(
    data[:, 0], data[:, 1], c=clusterer.labels_, cmap=custom_cmap, alpha=0.25, s=10
)
plt.title("HDBSCAN Clustering")
plt.show()
...
principalDf = pd.DataFrame(
    data=principalComponents, columns=[f"pc{i}" for i in range(1, 101)]
)
...
for target in targets:
    ...
    ax.scatter(
        finalDf.loc[indicesToKeep, "pc1"], finalDf.loc[indicesToKeep, "pc2"], s=20
    )
# ax.legend(targets)
ax.grid()
...
def plot_pca(ax, principal_df, title):
    ...
    for target in targets:
        ...
        ax.scatter(
            principal_df.loc[indices_to_keep, "pc1"],
...
principal_df_3 = pd.DataFrame(
    data=principal_components_3, columns=[f"pc{i}" for i in range(1, 4)]
)
...
principal_df_100 = pd.DataFrame(
    data=principal_components_100, columns=[f"pc{i}" for i in range(1, 101)]
)
...
principal_df_300 = pd.DataFrame(
    data=principal_components_300, columns=[f"pc{i}" for i in range(1, 301)]
)
