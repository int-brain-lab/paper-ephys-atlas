...
br = BrainRegions()
local_path = Path(
    "/Users/olivier/Documents/datadisk/paper-ephys-atlas/ephys-atlas-decoding/latest"
)
...
def aggregations_dictionary(fseries, nq=DEFAULT_NQ):
    ...
    aggregations = {
        ...
        f"{fseries.name}_gini": pd.NamedAgg(
            column=fseries.name, aggfunc=lambda x: _agg_gini(x, quantiles)
        ),
    }
    return aggregations
...
df_regions = df.groupby("atlas_id_beryl").agg(
    n_pids=pd.NamedAgg(column="atlas_id", aggfunc="count"), **aggregations
)
...
for feature in features:
    ...
    for aid in df["atlas_id_beryl"].unique():
        ...
        eq = np.nansum(
            -(p := f / np.sum(f, axis=1)[:, np.newaxis]) * np.log2(p), axis=1
        )
        remaining_information = np.sum(eq * np.sum(f, axis=1) / np.sum(f))
        df_regions.at[aid, f"{feature}_info_gain"] = (
            1 - remaining_information / region_entropy
        )
...
for feature in df_regions:
    ...
    print(feature)
    up.create_features(
        feature, acs, df_regions[feature].values.astype(np.double), hemisphere="left"
    )
