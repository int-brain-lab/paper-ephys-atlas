...
x_list += ['peak_time_idx', 'peak_val', 'trough_time_idx', 'trough_val', 'tip_time_idx', 'tip_val'] #Add this to give a total of 22 features as an input to the model
...
for pid in benchmark_pids:
    ...
    predict_regions_proba['beryl_aids'] = regions.remap(clf.classes_, source_map='Beryl', target_map='Beryl')
    predict_regions_proba['cosmos_aids'] = regions.remap(clf.classes_, source_map='Beryl', target_map='Cosmos')
    ...
    predictions_remap_cosmos = regions.remap(predict_region, source_map='Beryl', target_map='Cosmos')
    predictions_remap_beryl = regions.remap(predict_region, source_map='Beryl', target_map='Beryl')
    ...
    sns.set_theme()
    f, (ax1, ax2, ax3, ax4, ax5, ax6, ax7) = plt.subplots(1,7, figsize=(16,6), gridspec_kw={'width_ratios':[1.2, 1.2, 4.8, 1.5, 1.2, 1.2, 4.8]}, dpi=500)

 
    plot_brain_regions(df_pid['cosmos_id'], channel_depths=df_pid['axial_um'].values, 
                    brain_regions=regions, display=True, ax=ax1, title='Aligned Cosmos')
    ...
    plot_brain_regions(predictions_remap_cosmos, channel_depths=df_pid['axial_um'].values,
                            brain_regions=regions, display=True, ax=ax2, title='Predicted')
    ...
    plot_brain_regions(df_pid['beryl_id'], channel_depths=df_pid['axial_um'].values,
                            brain_regions=regions, display=True, ax=ax5, title='Aligned Beryl')
    ...
    plot_brain_regions(predictions_remap_beryl, channel_depths=df_pid['axial_um'].values,
                            brain_regions=regions, display=True, ax=ax6, title='Predicted')
    ...
    plt.subplots_adjust(left=0.1, bottom=0.1, right=0.9, top=0.85, wspace=0.30, hspace=0.8)
 
    # Remove white stripes on the probe plots 
    [c.get_children()[0].set_linewidth(0) for c in ax2.containers if isinstance(c, BarContainer)]
    [c.get_children()[0].set_linewidth(0) for c in ax6.containers if isinstance(c, BarContainer)]
