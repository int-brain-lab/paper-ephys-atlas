...
(np.ones_like(icc_lo), (icc_up, icc_up*0 )), shape=(ir_unique.size,1)).todense()

up2down = state_transitions / state_transitions.sum(axis=1)

np.savez(LOCAL_DATA_PATH.joinpath(f'region_transition_{str_mapping.lower()}.npz'),
         region_transitions=state_transitions, region_counts=states_count.squeeze(), region_aids=ba.regions.id[ir_unique]
         )
...
if str_mapping == 'Cosmos':
    sns.heatmap(up2down * 100, vmin=0, vmax=1.2, annot=ir_unique.size < 50, fmt='.1f', ax=ax, cmap='Blues')
    ax.set(
