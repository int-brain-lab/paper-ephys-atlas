...
def load_decoding(label="cosmos"):
    df_benchmarks = pd.read_parquet(
        FOLDER_GDRIVE.joinpath(f"{label}_predictions_benchmark.pqt")
    )
    ...
    df_depths = (
        df_benchmarks.reset_index()
        .groupby(["pid", "axial_um"])
        .agg(
            **{
                f"{label}_id": pd.NamedAgg(
                    column=f"{label   }_id", aggfunc=pd.Series.mode
                )
            },
            ...
            **{
                r: pd.NamedAgg(column=r, aggfunc="mean")
                for r in regions.id2acronym(classes)
            },
            **{r: pd.NamedAgg(column=r, aggfunc="mean") for r in features_list},
        )
    )
    df_depths[f"{label}_prediction"] = classes[
        np.argmax(df_depths.loc[:, regions.id2acronym(classes)].values, axis=1)
    ]
...
classes_priors = npz_transitions["region_counts"].squeeze() / np.sum(
    npz_transitions["region_counts"]
)
...
if label == "cosmos":
    ...
    sns.heatmap(
        emission * 100, vmin=0, vmax=50, cmap="Blues", annot=True, ax=ax, fmt=".1f"
    )
    ax.set(
...
for pid in pids:
    ...
    probas = df_depths.loc[
        pid, regions.id2acronym(classes)
    ].values  # this is the (ndepths, nregions) matrix of probabilities
    ...
    predictions = scipy.interpolate.interp1d(
        np.arange(predictions.size)[~irv],
        predictions[~irv],
        kind="nearest",
        fill_value="extrapolate",
    )(np.arange(predictions.size)).astype(int)
    probas[:, root_void] = 0
    ...
    for i in range(n_runs):
        ...
        match method:
            case "one_way":
                ...
                cl, p = viterbi(
                    emission, transition_down, init_hidden_probs, all_obs[:, i]
                )
                vprobs[i] = p
                all_vit[:, i] = cl
            case "two_ways":
                ...
                all_vit[dstart:nd, i], pdown = viterbi(
                    emission, transition_down, init_hidden_probs, all_obs[dstart:nd, i]
                )
    ...
    hacc = sklearn.metrics.accuracy_score(
        df_depths.loc[pid, f"{label}_id"].values, classes[vit_pred]
    )
    pacc = sklearn.metrics.accuracy_score(
        df_depths.loc[pid, f"{label}_id"].values,
        df_depths.loc[pid, f"{label}_prediction"].values,
    )
    vacc = sklearn.metrics.accuracy_score(
        df_depths.loc[pid, f"{label}_id"].values, classes[vit]
    )

    fig, axs = plt.subplots(
        1, 7, figsize=(16, 8), gridspec_kw={"width_ratios": [1, 1, 7, 1, 1, 1, 7]}
    )
    depths = df_depths.loc[pid].index.values
    plot_brain_regions(
        df_depths.loc[pid][f"{label}_id"].values,
        channel_depths=df_depths.loc[pid].index.values,
    ...
    plot_brain_regions(
        df_depths.loc[pid][f"{label}_prediction"],
        channel_depths=df_depths.loc[pid].index.values,
        brain_regions=regions,
        display=True,
        ax=axs[1],
    ...
    ephys_atlas.plots.plot_cumulative_probas(
        probas, depths, aids=classes, regions=regions, ax=axs[2], legend=False
    )
    ...
    rgbs = regions.rgb[
        ismember(np.sort(classes[all_vit.astype(int)], axis=1), regions.id)[1]
    ].reshape(nd, -1, 3)
...
acc_hmm = np.mean(
    df_depths.loc[pid, f"{label}_id"].values
    == df_depths.loc[pid, "hmm_stochastic"].values
)
acc_vit = np.mean(
    df_depths.loc[pid, f"{label}_id"].values == df_depths.loc[pid, "hmm"].values
)
acc = np.mean(
    df_depths.loc[pid, f"{label}_id"].values
    == df_depths.loc[pid, f"{label}_prediction"].values
...
## %%
df_acc = df_depths.loc[:, ["hmm", "hmm_stochastic", f"{label}_prediction"]].apply(
    lambda x: x == df_depths.loc[:, f"{label}_id"].values
)

df_acc_s = df_acc.groupby("pid").mean()
# df_acc_s = df_acc_s.apply(lambda x: x - df_acc_s['cosmos_prediction'])
df_acc_s = (
    df_acc_s.stack().reset_index().rename(columns={"level_1": "method", 0: "accuracy"})
)
