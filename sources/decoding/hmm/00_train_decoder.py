...
df_voltage, _, _, _ = ephys_atlas.data.load_voltage_features(
...
test_idx_folds = ephys_atlas.encoding.train_test_split_folds(
    df_voltage, include_benchmarks=True
)
...
for ifold, test_idx in enumerate(test_idx_folds):
    ...
    df_benchmarks = df_voltage.loc[
        ismember(df_voltage.index.get_level_values(0), ephys_atlas.data.BENCHMARK_PIDS)[
            0
        ],
        :,
    ].copy()
    ...
    match TRAIN_LABEL:
        case "Cosmos_id":  # Gradient boost
            ...
            df_benchmarks[f"cosmos_prediction"] = classes[
                classifier.predict(df_benchmarks.loc[:, x_list].values)
            ]
        ...
        case "Beryl_id":
            ...
            df_benchmarks["beryl_prediction"] = classifier.predict(
                df_benchmarks.loc[:, x_list].values
            )
            df_test["beryl_prediction"] = classifier.predict(
                df_test.loc[:, x_list].values
            )
            # null model
            # accuracy = sklearn.metrics.accuracy_score(
            #     df_voltage.loc[test_idx, TRAIN_LABEL].values,
    ...
    path_model = ephys_atlas.decoding.save_model(
        LOCAL_DATA_PATH.joinpath("models"),
        classifier,
        meta,
        subfolder=f"FOLD{ifold:02d}",
    )
...
df_voltage = df_voltage.merge(
    df_predictions.loc[:, columns], left_index=True, right_index=True, how="left"
)
...
print(
    f'aws --profile ibl s3 sync "{path_models}" s3://ibl-brain-wide-map-private/aggregates/atlas/models'
)
