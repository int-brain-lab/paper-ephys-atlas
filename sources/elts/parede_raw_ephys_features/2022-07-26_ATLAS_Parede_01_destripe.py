...
for i, apid in enumerate(alyx_pids):
    ...
    if destination.is_dir():
        if destination.joinpath(f'.version_{VERSION}').exists() or destination.joinpath(f'.01_destripe_{VERSION}').exists():
            continue
    print(i, f"python ./sources/ephys_atlas/rawephys.py destripe --pid {pid} --path {destination}")
    c += 1
    shutil.rmtree(destination, ignore_errors=True)
    ephys_atlas.rawephys.destripe(pid, one=one, destination=destination)
