...
def destripe(
    ...
    if typ == "ap":
        sample_duration, sample_spacings, skip_start_end = (
            10 * 30_000,
            1_000 * 30_000,
            500 * 30_000,
        )
    ...
    nsamples = np.ceil(
        (sr.shape[0] - sample_duration - skip_start_end * 2) / sample_spacings
    )
    assert (
        nsamples > 0
    ), f"Recording length is too small {sr.shape[0] / sr.fs: 0.2f} secs"
    t0_samples = (
        np.round((np.arange(nsamples) * sample_spacings + skip_start_end) / chunk_size)
        * chunk_size
    )
    ...
    for s0 in t0_samples:
        ...
        if typ == "ap":
            ...
            destripe = scipy.signal.decimate(
                destripe, LFP_RESAMPLE_FACTOR, axis=1, ftype="fir"
            )
        ...
        np.save(
            file_destripe.parent.joinpath(f"{typ}_raw.npy"),
            raw.astype(np.float16)[
                :, int(sr.fs * raw_sample_times[0]) : int(sr.fs * raw_sample_times[1])
            ],
        )
...
def localisation(destination=None, clobber=False):
    ...
    for i, ap_file in enumerate(all_files):
        ...
        for first, last in wg.firstlast:
            loc, wfs = subtract_and_localize_numpy(
                data[:, first:last].T, geom, **kwargs
            )
            cleaned_wfs = (
                wfs if first == 0 else np.concatenate([cleaned_wfs, wfs], axis=0)
            )
            loc["sample"] += first
...
def compute_lf_features(pid, root_path=None, bands=None, current_source=False):
    ...
    for i in np.arange(nfiles):
        ...
        df_chunk = ephys_atlas.features.lf(
            data, lf_info["fs"], bands=ephys_atlas.features.BANDS
        )
        df_chunks.append(df_chunk)
    ...
    lf_features = df_chunks.groupby("channel").agg(
        ...
        **{
            f"psd_{b}": pd.NamedAgg(column=f"psd_{b}", aggfunc="median")
            for b in ephys_atlas.features.BANDS
        },
    )
    return lf_features
