...
## Runs the destriping
report = workflow.report(one=one, pids=pids, root_path=ROOT_PATH)
...
for i, pid in enumerate(pids):
    print(i, len(pids))
    workflow.destripe_ap(pid=pid, one=one, root_path=ROOT_PATH)


pids = report.flow.get_pids_ready('destripe_lf', include_errors=RERUN_ERRORS)
for i, pid in enumerate(pids):
    print(i, len(pids))
    workflow.destripe_lf(pid=pid, one=one, root_path=ROOT_PATH)
...
for i, pid in enumerate(pids):
    print(i, len(pids))
    workflow.localise(pid, clobber=True, root_path=ROOT_PATH)
...
for i, pid in enumerate(pids):
    ...
    workflow.compute_sorted_features(pid, one=one, root_path=ROOT_PATH)

t = time.time()
pids = report.flow.get_pids_ready('compute_raw_features', include_errors=RERUN_ERRORS)
joblib.Parallel(n_jobs=18)(joblib.delayed(workflow.compute_raw_features)(pid, root_path=ROOT_PATH) for pid in pids)
