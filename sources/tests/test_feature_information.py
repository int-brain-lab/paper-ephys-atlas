...
import numpy as np
import scipy.stats
from ephys_atlas.feature_information import (
    feature_overall_entropy,
    feature_region_entropy,
)
...
def test_feature_overall_entropy(counts, nc):
    info_gain, entropy_feature, entropy_quantiles, entropy_overall = (
        feature_overall_entropy(counts, return_all=True)
    )
    np.testing.assert_array_almost_equal(
        entropy_quantiles, [0, 0, 1, 1.5849625, 1, 0], decimal=6
    )
...
def test_feature_region_entropy(counts, nc):
    ...
    info_gain_region, entropy_region, entropy_overall = feature_region_entropy(
        counts, return_all=True
    )
    ...
    np.testing.assert_array_almost_equal(
        entropy_region_hand, entropy_region, decimal=10
    )

    entropy_overall_hand = scipy.stats.entropy(
        np.array([0, 1, 4, 3, 2, 5]) / nc, base=2
    )
    np.testing.assert_array_almost_equal(entropy_overall_hand, entropy_overall)
