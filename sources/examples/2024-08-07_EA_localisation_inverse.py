...
match socket.gethostname():
    case 'little mac':
        # little mac
        LOCAL_DATA_PATH = Path('/Users/olivier/Documents/datadisk/Data/paper-ephys-atlas/ephys-atlas-decoding')
        NEMO_PATH = Path('/Users/olivier/Library/CloudStorage/GoogleDrive-olivier.winter@internationalbrainlab.org/Shared drives/Task Force - Electrophysiology Atlas/Decoding/NEMO')
    case 'parede':
        LOCAL_DATA_PATH = Path('/mnt/s0/ephys-atlas-decoding')
        NEMO_PATH = Path('/home/olivier/Insync/olivier.winter@internationalbrainlab.org/Google Drive - Shared drives/Task Force - Electrophysiology Atlas/Decoding/NEMO')
    case 'ferret':
        # work off linux
        LOCAL_DATA_PATH = Path("/datadisk/Data/paper-ephys-atlas/ephys-atlas-decoding")
        NEMO_PATH = Path("/datadisk/team_drives/Task Force - Electrophysiology Atlas/Decoding/NEMO")
...
brain_template = { 'soft_boundaries': npz['soft_boundaries'], 'cosmos_aids': npz['cosmos_aids'] }
...
_, _, _, df_probes = ephys_atlas.data.load_voltage_features(LOCAL_DATA_PATH.joinpath('features', '2024_W04'))
...
daggs = {k: pd.NamedAgg(column=k, aggfunc=lambda x: x.mode().iloc[0]) for k in columns_labels}
...
# %%
fig, axs = plt.subplots(1, 6, figsize=(16, 8), gridspec_kw={'width_ratios': [7, 1, 1, 1, 2, 7]})
fig.suptitle(f'Probe {pid}')
features_list = ephys_atlas.encoding.voltage_features_set()
sns.heatmap(scipy.stats.zscore(df_depths.loc[:, features_list]), ax=axs[0], vmin=-2, vmax=2, cmap='Spectral',
            cbar=True)
axs[0].set(title='z-scored features')
axs[0].invert_yaxis()
# axs[0].tick_params(axis='x', rotation=45)

plot_brain_regions(df_depths['Allen_id'].values, channel_depths=df_depths.index.values,
                   brain_regions=br, display=True, ax=axs[1], title='Allen')
plot_brain_regions(df_depths['Cosmos_id'].values, channel_depths=df_depths.index.values,
                   brain_regions=br, display=True, ax=axs[2], title='Real')
plot_brain_regions(df_depths['Cosmos_prediction'].values, channel_depths=df_depths.index.values,
                   brain_regions=br, display=True, ax=axs[3], title='Pred')

# ba.regions.to_df().set_index('id').loc[cosmos_aids].reset_index()
pr = brain_template['soft_boundaries'][ba._lookup(df_depths.loc[:, ['x', 'y', 'z']].values, mode='clip'), :]
...
ephys_atlas.plots.plot_cumulative_probas(
    probas=df_depths.loc[:, map(str, list(COSMOS_ID))].values, # (ndepths x nregions) array of probabilities for each region that sum to 1 for each depth
    depths=df_depths.index.values,
    aids=COSMOS_ID,
    regions=br,
    ax=axs[5],
...
ins = iblatlas.atlas.Insertion.from_track(df_pid.loc[:, ['x', 'y', 'z']].values, brain_atlas=ba)
...
ba.regions.id[ba.regions.mappings['Cosmos'][ba.label.flatten()[ba._lookup(xyz_svd, mode='clip')]]]


# this is the planned trajectory, what we will use in practice
ins_planned = iblatlas.atlas.Insertion.from_dict(dict(x=df_probes.loc[pid].x_target, y=df_probes.loc[pid].y_target, z=df_probes.loc[pid].z_target,
            phi=df_probes.loc[pid].phi_target, theta=df_probes.loc[pid].theta_target, depth=df_probes.loc[pid].depth_target), brain_atlas=ba)
...
opt = scipy.optimize.differential_evolution(loss, bounds=(
...
fig.suptitle(f'Probe {pid}')
ba.plot_cslice(ap_coordinate=np.median(df_pid['y']), volume='annotation', ax=axs[0], alpha=.7)
ba.plot_sslice(ml_coordinate=np.median(df_pid['x']), volume='annotation', ax=axs[1], alpha=.7)
...
axs[0].plot(df_pid['x'].values * 1e6, df_pid['z'].values * 1e6, '.r', label='aligned channels', **plot_args)
axs[1].plot(df_pid['y'].values * 1e6, df_pid['z'].values * 1e6, '.r', label='aligned channels', **plot_args)

axs[0].plot(ins_planned.xyz[:, 0] * 1e6, ins_planned.xyz[:, 2] * 1e6, label='planned trajectory', **plot_args)
axs[1].plot(ins_planned.xyz[:, 1] * 1e6, ins_planned.xyz[:, 2] * 1e6, label='planned trajectory', **plot_args)
axs[0].plot(xyz_svd[:, 0] * 1e6, xyz_svd[:, 2] * 1e6, label='forward trajectory', **plot_args)
axs[1].plot(xyz_svd[:, 1] * 1e6, xyz_svd[:, 2] * 1e6, label='forward trajectory', **plot_args)
...
mlab.plot3d(mlapdv[:, 1], mlapdv[:, 2], mlapdv[:, 0],
            line_width=1, tube_radius=20, color=color_cycle(0))
mlab.text3d(mlapdv[-1, 1], mlapdv[-1, 2], mlapdv[-1, 0] - 500, pid[:8],
            line_width=4, color=color_cycle(0), figure=fig, scale=150)
mlapdv = ba.xyz2ccf(xyz, mode='clip')
mlab.plot3d(mlapdv[:, 1], mlapdv[:, 2], mlapdv[:, 0],
            line_width=1, tube_radius=20, color=color_cycle(1))
