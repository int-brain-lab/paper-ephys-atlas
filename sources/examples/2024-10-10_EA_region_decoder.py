...
try:
    df_voltage, _, _, df_probes = ephys_atlas.data.load_voltage_features(
        LOCAL_DATA_PATH.joinpath("features", meta.VINTAGE)
    )
except FileNotFoundError:
    from one.api import ONE

    one = ONE(mode="remote")
    df_voltage, _, _, df_probes = ephys_atlas.data.download_tables(
        LOCAL_DATA_PATH.joinpath("features"), meta.VINTAGE, one=one
    )

np.sum(
    ~np.isin(test_pids, df_probes.index)
)  # hmm we have 3 probes from the BWM that are not in the voltage features

# Accuracy: 0.5509311541383488
brain_atlas = (
    ephys_atlas.anatomy.EncodingAtlas()
)  # Accuracy: 0.5459850465017324 - this atlas splits void and void fluid
...
if VOID_ROOT_HANDLING == "inflate":
    ...
    i2replace = (
        df_voltage["Cosmos_id"].isin(aids_replaced).values
    )  # we are going to get the nearest region of those root and void inside brain
    i_volume_flat = brain_atlas._lookup(
        df_voltage.loc[i2replace, ["x", "y", "z"]].values, mode="clip"
    )
    i_volume = np.c_[np.unravel_index(i_volume_flat, cosmos_volume.shape)]
    around = np.c_[
        [
            x.flatten().astype(int)
            for x in np.meshgrid([-1, 0, 1], [-1, 0, 1], [-1, 0, 1])
        ]
    ]
    n, nearest_rid, isearch = (
        0,
        np.zeros(np.sum(i2replace)),
        np.ones(np.sum(i2replace), dtype=bool),
    )
    while any(isearch):
        ...
        neighbours = np.take(
            ...
            np.ravel_multi_index(
                [iclose[:, i, :] for i in np.arange(3)],
                dims=cosmos_volume.shape,
                mode="clip",
            ),
        )
        neighbours_mask = ~np.isin(neighbours, rids_replaced)
        ifound = np.any(neighbours_mask, axis=1)
        nearest_rid[np.where(isearch)[0][ifound]] = neighbours[
            ifound, np.argmax(neighbours_mask[ifound, :], axis=1)
        ]
...
df_benchmarks = df_voltage.loc[
    ismember(df_voltage.index.get_level_values(0), ephys_atlas.data.BENCHMARK_PIDS)[0],
    :,
].copy()
