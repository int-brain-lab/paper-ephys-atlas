...
def compute_info_gain(
    df_voltage, feature, mapping, save_folder=None, save_name="info_gain"
):
    """
    ...
    df_entropy = pd.DataFrame(
        index=counts.index.rename(mapping + "_id_reg1"),
    ...
    if save_folder is not None:  # Save dataframe
        df_entropy.to_parquet(
            save_folder.joinpath(f"{feature}__{mapping}__info_gain.pqt")
        )
...
def compute_ks_test(df_voltage, feature, mapping, save_folder=None):
    ...
    # Create a dataframe of Nregion X Nregion that will contain the entropy computed for a pair of region
    df_ks = pd.DataFrame(
        index=pivot.index.rename(mapping + "_id_reg1"),
    ...
    for ireg1, reg1 in enumerate(pivot.index):
        for reg2 in pivot.index[ireg1 + 1 :]:
            ...
            df_ks.loc[
                (df_ks[mapping + "_id_reg1"] == reg1)
                & (df_ks[mapping + "_id_reg2"] == reg2),
                "statistic",
            ] = ks.statistic

            df_ks.loc[
                (df_ks[mapping + "_id_reg1"] == reg2)
                & (df_ks[mapping + "_id_reg2"] == reg1),
                "statistic",
            ] = ks.statistic
