...
def plot_kde(
    ...
    """
    if (
        regions_id is not None
    ):  # remove all the rows that aren't part of this set of region first
    ...
    caids, cids, _ = np.intersect1d(
        br.id, np.unique(df_voltage[brain_id]), return_indices=True
    )
    ...
    rec = patches.Rectangle(
        (summary.loc[feature, "q05"], 0),
        summary.loc[feature, "dq"],
        kpl.get_ylim()[1],
        alpha=0.1,
        color="k",
    )
    ...
    kpl.set(
        title=feature,
        xlim=summary.loc[feature, "median"]
        + summary.loc[feature, "dq"] * np.array([-0.5, 0.5]) * 2,
    )
    plt.tight_layout()
    if ax is None:
        return fig, ax
...
def plot_similarity_matrix(
    mat_plot, regions, fig=None, ax=None, br=None, cmap="viridis"
):
    """
...
def prepare_data_probe_plot(data_arr, xy, cmap=None, clim=None):
    ...
    if clim is None:
        clim = np.nanquantile(
            np.concatenate([np.squeeze(np.ravel(d)) for d in data_bank]).ravel(),
            [0.1, 0.9],
        )
    data.set_clim(clim)
    return data
...
def figure_features_chspace_probeplot(pid_df, features, xy):
    ...
    plot_brain_regions(
        pid_df["atlas_id"],
        channel_depths=pid_df["axial_um"].to_numpy(),
...
def plt_unit_acg(
    ...
    info_cell = df_clusters[
        ["uuids", "spike_count", "firing_rate", "label", "acronym"]
    ].iloc[i_cell]
    # Get auto-corr
    y = corr_rf[i_cell, :]
    x = np.arange(len(y))
    ...
    if x_time_ms:  # Plot in millisecond, otherwise in bin
        ax.set_xticklabels(
            np.around(x[0:-1:xstep].dot(bin_size_secs) * 1e3, decimals=2)
        )
    ax.set_title(
        f"uuids {info_cell.uuids[0:8]}, "
