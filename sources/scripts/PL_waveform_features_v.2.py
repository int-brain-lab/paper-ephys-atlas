...
if sys.platform == "linux":
    file_destripe = Path(
        "/Users/petrinalau/int-brain-lab/Data/f336f6a4-f693-4b88-b12c-c5cf0785b061_KS096_2022-06-21_001_probe00/chunk0500_destripe.npy"
    )
else:
    file_destripe = Path(
        "/Users/petrinalau/int-brain-lab/Data/f336f6a4-f693-4b88-b12c-c5cf0785b061_KS096_2022-06-21_001_probe00/chunk0500_destripe.npy"
...
waveforms = np.load(
    file_destripe.parent.joinpath(file_destripe.stem + "_waveforms.npy")
)
df_spikes["r"] = np.abs(
    xy[df_spikes["trace"].values.astype(np.int32)]
    - df_spikes["x"]
    - 1j * df_spikes["y"]
)
...
cind = (
    r <= kwargs["extract_radius"]
)  # index of channel of the waveform inside the raw data on selected channel that we can extract waveform from
...
wiggle(-data[cind, sind].T, fs=rinfo["fs"], gain=0.25, ax=axs[1])
...
if spike_width <= 0.08:
    ...
    (
        ...
        x_time[np.argmin(w) : peak_after_trough], w[np.argmin(w) : peak_after_trough], 1
    )
...
for k, eqc in eqcs.items():
    eqc.ctrl.add_scatter(
        df_spikes["sample"].values / fs * 1e3, df_spikes["trace"].values, label="loc"
    )
