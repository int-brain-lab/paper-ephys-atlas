...
local_data_path_ks = Path(
    f"/Users/gaelle/Documents/Work/EphysAtlas/Entropy_KS/{label}/"
)
local_data_path_en = Path(
    f"/Users/gaelle/Documents/Work/EphysAtlas/Entropy_DF_WF/{label}/"
)
...
df_info_ks = pd.read_parquet(
    local_data_path_ks.joinpath(f"{label}__{mapping}__overall_ks_test.pqt")
)
df_info_en = pd.read_parquet(
    local_data_path_en.joinpath(f"{label}__{mapping}__overall_info_gain.pqt")
)
...
# Save
information_gain_ks.to_parquet(
    save_folder.joinpath(f"{label}__{mapping}__information_gain_ks.pqt")
)
information_gain_en.to_parquet(
    save_folder.joinpath(f"{label}__{mapping}__information_gain_en.pqt")
)
...
df_kdt = pd.DataFrame(
    index=information_gain_ks.index.rename(mapping + "_id"),
    columns=["kdt_correlation", "kdt_pvalue", "kdt_statistic", "cayley_dist"],
)
...
for br_id in information_gain_ks.index:
    ...
    correlation = stats.kendalltau(
        df_merge["entropy_argsort"].to_list(),
        df_merge["ks_statistic_argsort"].to_list(),
    )

    cayley_dist = cayley_distance(
        df_merge["entropy_argsort"].to_list(),
        df_merge["ks_statistic_argsort"].to_list(),
    )
    # Save
    df_kdt.loc[(df_kdt[mapping + "_id"] == br_id), "kdt_correlation"] = (
        correlation.correlation
    )
    df_kdt.loc[(df_kdt[mapping + "_id"] == br_id), "kdt_pvalue"] = correlation.pvalue
    df_kdt.loc[(df_kdt[mapping + "_id"] == br_id), "kdt_statistic"] = (
