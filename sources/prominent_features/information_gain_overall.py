...
information_gain = (
    pd.DataFrame(information_gain, index=["information_gain"])
    .T.sort_values("information_gain", ascending=False)
    .reset_index()
)
##
...
fig, ax = plt.subplots()
sns.barplot(
    information_gain, y="information_gain", x="index", palette=information_gain["color"]
)
