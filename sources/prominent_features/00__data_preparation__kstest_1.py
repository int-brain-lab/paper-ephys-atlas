...
if brain_id == "cosmos_id":
    regions = br.acronym2id(
        ["Isocortex", "HPF", "CTXsp", "OLF", "CNU", "HY", "TH", "MB", "HB", "CB"]
    )
...
for i_reg, reg in enumerate(regions):  # Fill in the array
    reg_arr[i_reg, :, 0] = reg
    reg_arr[:, i_reg, 1] = (
        reg  # cf TODO Note: this is not used currently but could be useful if re-written
    )
...
for i_reg, reg in enumerate(regions):
    reg_comp_arr = reg_arr[
        i_reg + 1 :, i_reg, 0
    ]  # Regions to compare against start from the next available
...
if USE_DEBIAS:
    local_result_b = (
        local_result.joinpath(brain_id)
        .joinpath("use_debias")
        .joinpath(label)
        .joinpath(test_todo)
    )
