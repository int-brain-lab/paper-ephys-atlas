...
for i_f, feature in enumerate(features):
    ...
    if i_f == 0:
        ...
        information_gain_region[mapping + "_acronym"] = br.id2acronym(
            information_gain_region.index.values, mapping=mapping
        )
    information_gain_region[feature] = feature_region_entropy(counts, normalise=True)

information_gain_region = information_gain_region.merge(
    df_regions[["n_channels"]], left_index=True, right_index=True
)
